...
# ******** bark on person when ever dog find person in park  ********
                   
        elif action == "bark":
            items = self.list_things_at(agent.location, tclass=Person)
            if len(items) != 0:
                if agent.bark(items[0]): #Have the dog bark first item
                    print('{} bark on {} at location: {}'
                          .format(str(agent)[1:-1], str(items[0])[1:-1], agent.location))
                    self.delete_thing(items[0]) #Delete it from the Park after.
...
# ********** ********** *********** ********** ************ *********** ********* *******
    ...
    def is_done(self):
        '''By default, we're done when we can't find a live agent, 
        but to prevent killing our cute dog, we will stop before itself - when there is no more food or water or Person.'''
...
def program(percepts):
    ...
    for p in percepts:
        ...
        # ++++++++++ if dog find person it will return to bark  ++++++++
...
# add food on location 9
bone = Food()                
park.add_thing(bone,9)

# create object of person class
Parth = Person()
Patel = Person()

# set Person object at 3 & 12 location in park
park.add_thing(Parth,3)
park.add_thing(Patel,12)
